...
// setup the dpixa client, you can use other nodes, for example anyx's public node at https://anyx.io
const client = new dpixa.Client('https://api.pixagram.io')

// deserialize the posting key (in wif format, same format as you find on the pixagram.io interface)
const key = dpixa.PrivateKey.from(POSTING_KEY)
...
const stream = client.blockchain.getOperationsStream({mode: dpixa.BlockchainMode.Latest})
    ...
    if (operation.op[0] == 'vote') {
        ...
        if (vote.voter === FOLLOW_USER) {
            ...
            client.broadcast.vote(vote, key).then(() => {
                console.log(`Voted for https://pixagram.io/@${ vote.author }/${ vote.permlink }`)
            }).catch((error) => {
