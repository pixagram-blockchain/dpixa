...
describe("asset", function() {
  it("should create from string", function() {
    const onePixa = Asset.fromString("1.000 PIXA");
    assert.equal(onePixa.amount, 1);
    assert.equal(onePixa.symbol, "PIXA");
    const vests = Asset.fromString("0.123456 VESTS");
    assert.equal(vests.amount, 0.123456);
    assert.equal(vests.symbol, "VESTS");
    const pxs = Asset.from("0.444 PXS");
    assert.equal(pxs.amount, 0.444);
    assert.equal(pxs.symbol, "PXS");
  });

  it("should convert to string", function() {
    const hive = new Asset(44.999999, "PIXA");
    assert.equal(hive.toString(), "45.000 PIXA");
    const vests = new Asset(44.999999, "VESTS");
    assert.equal(vests.toString(), "44.999999 VESTS");
  });

  it("should add and subtract", function() {
    const a = new Asset(44.999, "PIXA");
    assert.equal(a.subtract(1.999).toString(), "43.000 PIXA");
    assert.equal(a.add(0.001).toString(), "45.000 PIXA");
    assert.equal(
      Asset.from("1.999 PIXA")
        .subtract(a)
        .toString(),
      "-43.000 PIXA"
    );
    ...
    assert.throws(() =>
      Asset.fromString("100.000 PIXA").subtract("100.000000 VESTS")
    );
  ...
  it("should throw on invalid values", function() {
    ...
    assert.throws(() => Asset.fromString("Infinity PIXA"));
    assert.throws(() => Asset.fromString("..0 PIXA"));
    assert.throws(() => Asset.from("..0 PIXA"));
  ...
  it("should parse price", function() {
    const price1 = new Price(Asset.from("1.000 PIXA"), Asset.from(1, "PXS"));
    const price2 = Price.from(price1);
    const price3 = Price.from({ base: "1.000 PIXA", quote: price1.quote });
    assert.equal(price1.toString(), "1.000 PIXA:1.000 PXS");
  ...
  it("should get vesting share price", function() {
    const props: any = {
      total_vesting_fund_hive: "5.000 PIXA",
    ...
    assert.equal(price1.quote.symbol, "PIXA");
    const badProps: any = {
      total_vesting_fund_hive: "0.000 PIXA",
    ...
    assert.equal(price2.quote.symbol, "PIXA");
  });

  it("should convert price", function() {
    const price1 = new Price(Asset.from("0.500 PIXA"), Asset.from("1.000 PXS"));
    const v1 = price1.convert(Asset.from("1.000 PIXA"));
    assert.equal(v1.amount, 2);
    assert.equal(v1.symbol, "PXS");
    const v2 = price1.convert(Asset.from("1.000 PXS"));
    assert.equal(v2.amount, 0.5);
    assert.equal(v2.symbol, "PIXA");
    assert.throws(() => {
